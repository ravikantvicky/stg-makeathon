...
public interface EventsRepository extends JpaRepository<Events, Integer> {
	...
	@Query("select e from Events e where STR_TO_DATE(concat(e.eventDate, ' ', e.eventTime), '%Y-%m-%d %H:%i:%s') > current_timestamp() and ((upper(e.keywords) like upper(?1)) or (upper(e.name) like upper(?1))) and upper(location) = upper(?2)")
	public List<Events> findByKeywordAndLocation(String keyword, String location);

	@Query("select e from Events e where STR_TO_DATE(concat(e.eventDate, ' ', e.eventTime), '%Y-%m-%d %H:%i:%s') > current_timestamp() and ((upper(e.keywords) like upper(?1)) or (upper(e.name) like upper(?1)))")
